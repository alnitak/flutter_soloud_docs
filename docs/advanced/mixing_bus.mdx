...
// Play sounds through the bus (supports scale, looping, and loop regions)...
final sfxHandle = sfxBus.play(explosion);
final musicHandle = musicBus.play(
  backgroundMusic,
  looping: true,
  scale: 1.0,
);
...
final explosionHandle = SoLoud.instance.play(explosion, busId: sfxBus.busId);
```
