...
The *Opus* and *Ogg* libraries are embedded by default in flutter_soloud. However, if you don't need streaming capabilities, please read the [Without Opus/Ogg](/get_started/no_xiph_libs) section for how to exclude these libraries from your app.
...
| `format`               | Audio data format. Options: `f32le`, `s8`, `s16le`, `s32le`, or`auto`. <br/>**Note:** the `auto` autodetect MP3, WAV, FLAC and Ogg container with Opus, Vorbis, and FLAC. With this format, the `samplerate` and `channels` parameters are ignored.|
