...
*flutter_soloud* supports streaming audio data while receiving it in real-time. The supported audio data formats are raw PCM, MP3, WAV, FLAC, AAC (ADTS), AC-3, E-AC-3, or compressed through the [Opus](https://www.opus-codec.org/), [Vorbis](https://xiph.org/vorbis/), and [FLAC](https://xiph.org/flac/) codecs with [Ogg](https://xiph.org/ogg/) container libraries from [Xiph.org](https://www.xiph.org/). This is particularly useful when:
- Streaming audio from network sources like icecast radio or WebSockets
- Working with OpenAI or other streaming audio APIs
...
<Warning>
**MP4 and M4A containers are NOT supported for streaming:**
ISO base media file format containers (`.mp4`, `.m4a`) require non-sequential container/moov atom parsing and are therefore **not supported** for streaming (`setBufferStream` or `setPullBufferStream`). Use file/memory loading (`loadAsset`, `loadFile`, `loadMem`) for MP4 and M4A audio. For real-time streaming, stream raw AAC (ADTS streams), MP3, Ogg (Opus/Vorbis/FLAC), WAV, FLAC, AC-3, or E-AC-3.
</Warning>
...
| `sampleRate`           | Sample rate for playback (e.g., 22050 or 44100 Hz). Used for raw PCM streams. When using `BufferType.auto`, incoming audio metadata will automatically configure the sample rate. |
| `channels`             | Number of audio channels. Used for raw PCM streams. When using `BufferType.auto`, channels are detected automatically from the stream. |
| `format`               | Audio data format. Options: `f32le`, `s8`, `s16le`, `s32le`, or `auto`. <br/>**Note:** `BufferType.auto` autodetects MP3, WAV, FLAC, AAC (ADTS), AC-3, E-AC-3, and Ogg containers with Opus, Vorbis, and FLAC. With this format, the `sampleRate` and `channels` parameters are ignored. *(Note: MP4 and M4A containers are not supported for streaming).* |
...
You also need to specify the sample rate and the number of channels for raw PCM data.

- `auto` - Automatically detect the format

Supported formats in `auto` mode are:
- `mp3` - MP3 format
- `wav` - WAV format
- `flac` - FLAC format
...
- `aac` - AAC format (raw ADTS stream)
- `ac3` - AC-3 (Dolby Digital, where platform supported)
- `eac3` - E-AC-3 (Dolby Digital Plus, where platform supported)

These compressed formats support metadata (using the `onMetadata` callback).

<Warning>
**MP4 and M4A containers are NOT supported for streaming:**
Streaming (`setBufferStream` and `setPullBufferStream`) requires packet-based or streamable audio formats. Because ISO base media file format containers (`.mp4`, `.m4a`) require seeking / moov atom index parsing, they cannot be streamed in chunks. Load them as files/assets (`loadAsset`, `loadFile`, `loadMem`) instead.
</Warning>

<Info>
**Linux Note for AAC, AC-3, and E-AC-3:** Streaming these formats on Linux requires FFmpeg runtime libraries (`libavcodec` and `libavformat`) installed on the user's system (e.g. `sudo apt install ffmpeg libavcodec-extra`, `sudo pacman -S ffmpeg`, `sudo dnf install ffmpeg-free`). Core formats (MP3, WAV, FLAC, OGG) do not require FFmpeg.
</Info>
