...
- **iOS**: Links universal static `.a` archives covering physical devices (`arm64`) and simulators (`arm64`, `x86_64`) directly into the plugin binary.
- **macOS**: Links universal static `.a` archives supporting both Apple Silicon (`arm64`) and Intel (`x86_64`) directly into `libflutter_soloud.dylib`. This ensures the `.app` or `.dmg` is self-contained and never crashes on user machines that lack Homebrew.
- **Windows**: Bundles precompiled 64-bit (`x86_64`) `.dll` files next to the compiled executable. *(For Windows ARM64, use `windows_use_system_libs: true` or `windows_force_build_libs: true`)*.
- **Linux**: Bundles precompiled 64-bit (`x86_64`) `.so` files alongside the application. *(For ARM Linux / Raspberry Pi, use `linux_use_system_libs: true` or `linux_force_build_libs: true`)*.
