...
The web build uses a WebAssembly module that requires `SharedArrayBuffer`, which means it must be run with `--wasm` or served with cross-origin isolation headers. See the [Web Platform Guide](/get_started/web_notes) for details.
