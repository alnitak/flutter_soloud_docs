...
* `xiph`: Contains the script to build `ogg`, `opus`, `vorbis`, and `flac` libraries for all the platforms.
...
On macOS and iOS, the [Swift Package Manager](https://docs.flutter.dev/packages-and-plugins/swift-package-manager/for-app-developers) (SPM) can be used to build the plugin module. If SPM is not enabled, the build will be done using CocoaPods (using `cmake`).
