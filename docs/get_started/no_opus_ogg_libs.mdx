...
If `NO_OPUS_OGG_LIBS` is not set and therefore libraries are linked, by default the build will look at the system's shared libraries for Ogg, Opus, Vorbis, and FLAC. It will fall back to the embedded libraries if they are not found.

A new environment variable could be set to to force the linking of the embedded libraries. Here a brief usage:
```bash
# Default - prefer system, fallback to bundled
flutter build linux

# Force precompiled libraries
FORCE_PRECOMPILED_LIBS=1 flutter build linux
