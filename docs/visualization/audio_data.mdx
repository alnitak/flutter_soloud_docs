...
- `wavePointers` / `fftPointers`: `List<Pointer<Float>>?` providing direct zero-copy access to the underlying native C++ memory buffers (FFI only, `null` on Web).
- `waveData` / `fftData`: Convenience getters for single-channel / merged audio data (`wave.first` / `fft.first`).
- `wavePointer` / `fftPointer`: Convenience getters for the first native pointer.
