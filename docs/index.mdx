...
### Core Audio
- ⚡ Low latency, high performance playback with optional render-ahead ring (native)
- 🌐 Multi-threaded WebAudio AudioWorklet rendering on web
...
- 🔴 Mixer output capture for recording or streaming

### Format Support
- 🎵 MP3, WAV, OGG, and FLAC playback, plus native platform decoding for M4A, MP4 (audio tracks), AAC, AC-3, and E-AC-3
...
- 🔄 Buffered audio streaming with:
  ...
  - MP3, WAV, FLAC, Ogg (Vorbis/Opus/FLAC), AAC (ADTS), AC-3, and E-AC-3 formats supported *(Note: MP4 and M4A containers are not supported for streaming)*
